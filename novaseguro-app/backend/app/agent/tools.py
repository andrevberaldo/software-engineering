...
`build_tools(tenant_id)` é uma factory: cada tool é definida dentro dela e
fecha sobre `tenant_id`, garantindo que toda consulta ao banco fique presa
ao tenant do usuário logado — sem isso, `buscar_documentos` (busca vetorial)
vazaria documentos de um assinante para outro, e as demais tools aceitariam
ids de apólice de qualquer tenant.
"""
...
def build_tools(tenant_id: int) -> list:
    ...
    def buscar_documentos(pergunta: str, top_k: int = 4) -> str:
        ...
        with get_cursor() as cur:
            cur.execute(
                ...
                JOIN documentos d ON d.id = c.documento_id AND d.tenant_id = c.tenant_id
                WHERE c.embedding IS NOT NULL AND c.tenant_id = %(tenant_id)s
                ORDER BY c.embedding <=> %(q)s
                LIMIT %(k)s
                """,
                {"q": query_embedding, "k": top_k, "tenant_id": tenant_id},
            )
            rows = cur.fetchall()
    ...
    def consultar_cliente(nome_ou_email: str) -> str:
        """Consulta os dados de um cliente da corretora: suas apólices ativas,
        ...
        with get_cursor() as cur:
            cur.execute(
                ...
                WHERE tenant_id = %(tenant_id)s
                  AND (nome ILIKE %(termo)s OR email ILIKE %(termo)s)
                LIMIT 1
                """,
                {"termo": f"%{nome_ou_email}%", "tenant_id": tenant_id},
            )
            ...
            cur.execute(
                ...
                WHERE a.cliente_id = %(cid)s AND a.tenant_id = %(tenant_id)s
                ORDER BY a.data_renovacao
                """,
                {"cid": cliente["id"], "tenant_id": tenant_id},
            )
            ...
            cur.execute(
                ...
                FROM sinistros sin
                JOIN apolices a ON a.id = sin.apolice_id AND a.tenant_id = sin.tenant_id
                WHERE a.cliente_id = %(cid)s AND sin.tenant_id = %(tenant_id)s
    ...
    def prever_renovacao_apolice(apolice_id: int) -> str:
        ...
        with get_cursor() as cur:
            try:
                resultado = compute_prediction(cur, apolice_id, tenant_id)
    ...
    def listar_apolices_em_risco(dias: int = 30) -> str:
        ...
        with get_cursor() as cur:
            ...
            resultados = [compute_prediction(cur, apolice_id, tenant_id) for apolice_id in ids]
    ...
    def registrar_contato_renovacao(apolice_id: int, mensagem: str, canal: str = "email") -> str:
        ...
        with get_cursor() as cur:
            try:
                previsao = compute_prediction(cur, apolice_id, tenant_id)
            except ValueError as exc:
                return str(exc)
            ...
            cur.execute(
                """
                INSERT INTO contatos_renovacao (tenant_id, apolice_id, canal, mensagem, motivo)
                VALUES (%(tenant_id)s, %(apolice_id)s, %(canal)s, %(mensagem)s, %(motivo)s)
